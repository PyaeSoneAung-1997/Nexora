...
class NexoraUI(tk.Tk):
    """Tkinter Test UI Class for Live Sync Monitoring"""
    ...
    def _init_ui(self):
        ...
        self.combo_type = ttk.Combobox(input_frame, values=["my_drive", "shared_drive"], width=10, state="readonly")
        self.combo_type.set("my_drive")
        self.combo_type.grid(row=1, column=1, padx=10, pady=8)

        # 2. Controls Section
        btn_frame = ttk.LabelFrame(self, text=" Sync Thread Controls ")
        btn_frame.pack(fill="x", padx=15, pady=5)

        ttk.Button(btn_frame, text="▶️ Start / Resume", command=self.start_sync).grid(row=0, column=0, padx=8, pady=10)
