...
class AddUrlDialog(tk.Toplevel):
    ...
    def _init_ui(self):
        center_relative_to_parent(self,width=500, height=150)

        frame = ttk.LabelFrame(self, text=" Target Drive Information ")
        frame.pack(fill="both", expand=True, padx=10, pady=10)

        # 1. URL / ID Input
        ttk.Label(frame, text="Add URL:").pack(anchor="w", padx=10, pady=(10, 2))

        input_frame = ttk.Frame(frame)
        input_frame.pack(fill="x", padx=10, pady=5)

        self.ent_url = ttk.Entry(input_frame, width=10)
        self.ent_url.pack(fill="x", side="left", expand=True, padx=10, pady=5)
        
        ttk.Button(input_frame,text="Go",command=self._process_url).pack(side="right")
    ...
    def _process_url(self):
        url_mg = URLManager()
        check_url = self.ent_url.get().strip()
        ...
        result = url_mg.analyze(check_url)
        extracted_id = result['type'] # 'folder' or 'file'
        # target_type
        if not extracted_id:
